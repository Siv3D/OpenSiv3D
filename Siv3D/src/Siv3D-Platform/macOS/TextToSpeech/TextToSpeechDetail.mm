...
namespace s3d
{
	...
	bool CTextToSpeech::CTextToSpeechDetail::hasLanguage(const int32 languageCode)
	{
		return m_detail->hasLanguage(languageCode);
	}
	
	void CTextToSpeech::CTextToSpeechDetail::setDefaultLanguage(const int32 languageCode)
	{
		m_detail->setDefaultLanguage(languageCode);
	}
	
	int32 CTextToSpeech::CTextToSpeechDetail::getDefaultLanguage() const
	{
		return m_detail->getDefaultLanguage();
	}
	
	bool CTextToSpeech::CTextToSpeechDetail::speak(const std::string& text, const int32 languageCode)
	{
