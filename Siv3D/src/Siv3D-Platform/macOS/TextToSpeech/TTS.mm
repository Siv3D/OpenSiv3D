...
namespace s3d
{
	...
	bool TTS::hasLanguage(const int32 languageCode)
	{
	...
	void TTS::setDefaultLanguage(const int32 languageCode)
	{
		m_defaultLanguageCode = languageCode;
	}
	
	int32 TTS::getDefaultLanguage() const
	{
		return m_defaultLanguageCode;
	}
	
	bool TTS::speak(const std::string& text, int32 languageCode)
	{
		reset();
