...
namespace s3d
{
	...
	void WebGPUInternalTexture2D::clear(const ColorF color)
	{
		if (isEmpty())
		{
			return;
	...
	GLuint WebGPUInternalTexture2D::getFrameBuffer() const noexcept
	{
		return m_frameBuffer;
	}

	GLuint WebGPUInternalTexture2D::getTexture() const noexcept
	{
		return m_texture;
	}

	std::unique_ptr<WebGPUInternalTexture2D> WebGPUInternalTexture2D::CreateRenderTargetTexture2D(const Size size, const uint32 sampleCount)
	{
