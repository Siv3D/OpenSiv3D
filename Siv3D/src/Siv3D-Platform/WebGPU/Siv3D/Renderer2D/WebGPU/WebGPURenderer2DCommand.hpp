...
namespace s3d
{
	enum class WebGPURenderer2DCommandType : uint32
	{
		Null,
	...
	static_assert(FromEnum(WebGPURenderer2DCommandType::SIZE_) < 64);

	struct WebGPURenderer2DCommand
	{
		WebGPURenderer2DCommandType type = WebGPURenderer2DCommandType::Null;

		uint32 index = 0;
		
		WebGPURenderer2DCommand() = default;
		
		constexpr WebGPURenderer2DCommand(WebGPURenderer2DCommandType _type, uint32 _index) noexcept
			: type(_type)
			, index(_index) {}
	};

	struct WebGPUDrawCommand
	{
		uint32 indexCount = 0;
	};

	struct WebGPUConstantBufferCommand
	{
	...
	class WebGPURenderer2DCommandManager
	{
	private:

		// commands
		Array<WebGPURenderer2DCommand> m_commands;
		CurrentBatchStateChanges<WebGPURenderer2DCommandType> m_changes;

		// buffer
		Array<WebGPUDrawCommand> m_draws;
		...
		Array<WebGPUConstantBufferCommand> m_constantBufferCommands;
		...
		// current
		WebGPUDrawCommand m_currentDraw;
	...
	public:
		...
		WebGPURenderer2DCommandManager();

		void reset();

		void flush();

		const Array<WebGPURenderer2DCommand>& getCommands() const noexcept;

		void pushUpdateBuffers(uint32 batchIndex);

		void pushDraw(Vertex2D::IndexType indexCount);
		const WebGPUDrawCommand& getDraw(uint32 index) const noexcept;
		...
		WebGPUConstantBufferCommand& getConstantBuffer(uint32 index);
