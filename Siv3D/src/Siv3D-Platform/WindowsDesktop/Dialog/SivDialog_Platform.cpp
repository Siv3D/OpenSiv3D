...
namespace s3d
{
	...
	namespace Dialog
	{
		Optional<FilePath> OpenFile(const Array<FileFilter>& filters, const FilePath& defaultPath, const String& title)
		{
			...
			if (HRESULT result = fileOpenDialog->Show(static_cast<HWND>(Siv3DEngine::Get<ISiv3DWindow>()->getHandle()));
				SUCCEEDED(result))
			{
		...
		Optional<FilePath> SaveFile(const Array<FileFilter>& filters, const FilePath& defaultPath, const String& title)
		{
			...
			if (HRESULT result = fileSaveDialog->Show(static_cast<HWND>(Siv3DEngine::Get<ISiv3DWindow>()->getHandle()));
				SUCCEEDED(result))
			{
				ComPtr<IShellItem> shellItem;
		...
		Optional<FilePath> SelectFolder(const FilePath& defaultPath, const String& title)
		{
			...
			if (HRESULT result = folderDialog->Show(static_cast<HWND>(Siv3DEngine::Get<ISiv3DWindow>()->getHandle()));
				SUCCEEDED(result))
			{
				ComPtr<IShellItem> shellItem;
