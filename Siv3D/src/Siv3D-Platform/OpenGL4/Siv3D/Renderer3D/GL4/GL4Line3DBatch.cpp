...
namespace s3d
{
	...
	VertexLine3DBufferPointer GL4Line3DBatch::requestBuffer(const VertexLine3D::IndexType vertexSize, const uint32 indexSize, GL4Renderer3DCommandManager& commandManager)
	{
		// VB
			...
			m_vertexArray.size() < vertexArrayWritePosTarget) SIV3D_UNLIKELY
		{
			...
			LOG_TRACE(U"ℹ️ Resized GL4Line3DBatch::m_vertexArray (size: {} -> {})"_fmt(m_vertexArray.size(), newVertexArraySize));
			...
			m_indexArray.size() < indexArrayWritePosTarget) SIV3D_UNLIKELY
		{
			...
			LOG_TRACE(U"ℹ️ Resized GL4Line3DBatch::m_indexArray (size: {} -> {})"_fmt(m_indexArray.size(), newIndexArraySize));
	...
	size_t GL4Line3DBatch::num_batches() const noexcept
	{
		return m_batches.size();
	}

	bool GL4Line3DBatch::isEmpty() const noexcept
	{
		return (m_vertexArrayWritePos == 0);
	}

	void GL4Line3DBatch::reset()
	{
		m_batches.clear();
		m_batches.emplace_back();
	...
	void GL4Line3DBatch::setBuffers()
	{
		::glBindVertexArray(m_vao);
		::glBindBuffer(GL_ARRAY_BUFFER, m_vertexBuffer);
	}

	BatchInfoLine3D GL4Line3DBatch::updateBuffers(const size_t batchIndex)
	{
	...
	void GL4Line3DBatch::advanceArrayWritePos(const VertexLine3D::IndexType vertexSize, const uint32 indexSize) noexcept
	{
