...
namespace s3d
{
	...
	void GL4Renderer2DCommandManager::reset()
	{
		...
		// Begin a new frame
		{
			...
			m_commands.emplace_back(GL4Renderer2DCommandType::ColorMul, 0);
			m_currentColorMul = m_colorMuls.front();

			m_commands.emplace_back(GL4Renderer2DCommandType::ColorAdd, 0);
			...
			m_commands.emplace_back(GL4Renderer2DCommandType::ScissorRect, 0);
			m_currentScissorRect = m_scissorRects.front();

			m_commands.emplace_back(GL4Renderer2DCommandType::Viewport, 0);
	...
	void GL4Renderer2DCommandManager::flush()
	{
		...
		if (m_changes.has(GL4Renderer2DCommandType::ColorMul))
		{
			m_commands.emplace_back(GL4Renderer2DCommandType::ColorMul, static_cast<uint32>(m_colorMuls.size()));
			m_colorMuls.push_back(m_currentColorMul);
		}

		if (m_changes.has(GL4Renderer2DCommandType::ColorAdd))
		{
			m_commands.emplace_back(GL4Renderer2DCommandType::ColorAdd, static_cast<uint32>(m_colorAdds.size()));
		...
		if (m_changes.has(GL4Renderer2DCommandType::ScissorRect))
		{
			m_commands.emplace_back(GL4Renderer2DCommandType::ScissorRect, static_cast<uint32>(m_scissorRects.size()));
			m_scissorRects.push_back(m_currentScissorRect);
		}

		if (m_changes.has(GL4Renderer2DCommandType::Viewport))
		{
			m_commands.emplace_back(GL4Renderer2DCommandType::Viewport, static_cast<uint32>(m_viewports.size()));
		...
		{
			m_commands.emplace_back(GL4Renderer2DCommandType::SetRT, static_cast<uint32>(m_RTs.size()));
		...
		if (m_changes.has(GL4Renderer2DCommandType::SetConstantBuffer))
		{
	...
	void GL4Renderer2DCommandManager::pushColorMul(const Float4 & color)
	{
		constexpr auto command = GL4Renderer2DCommandType::ColorMul;
	...
	const Float4& GL4Renderer2DCommandManager::getColorMul(const uint32 index) const
	{
		return m_colorMuls[index];
	}

	const Float4& GL4Renderer2DCommandManager::getCurrentColorMul() const
	{
		return m_currentColorMul;
	}

	void GL4Renderer2DCommandManager::pushColorAdd(const Float4& color)
	{
		constexpr auto command = GL4Renderer2DCommandType::ColorAdd;
	...
	const Float4& GL4Renderer2DCommandManager::getColorAdd(const uint32 index) const
	{
		return m_colorAdds[index];
	}

	const Float4& GL4Renderer2DCommandManager::getCurrentColorAdd() const
	{
	...
	void GL4Renderer2DCommandManager::pushScissorRect(const Rect & state)
	{
		constexpr auto command = GL4Renderer2DCommandType::ScissorRect;
	...
	const Rect& GL4Renderer2DCommandManager::getScissorRect(const uint32 index) const
	{
		return m_scissorRects[index];
	}

	const Rect& GL4Renderer2DCommandManager::getCurrentScissorRect() const
	{
		return m_currentScissorRect;
	}

	void GL4Renderer2DCommandManager::pushViewport(const Optional<Rect>& state)
	{
		constexpr auto command = GL4Renderer2DCommandType::Viewport;
	...
	const Optional<Rect>& GL4Renderer2DCommandManager::getViewport(const uint32 index) const
	{
		return m_viewports[index];
	}

	const Optional<Rect>& GL4Renderer2DCommandManager::getCurrentViewport() const
	{
	...
	void GL4Renderer2DCommandManager::pushConstantBuffer(const ShaderStage stage, const uint32 slot, const ConstantBufferBase& buffer, const float* data, const uint32 num_vectors)
	{
		constexpr auto command = GL4Renderer2DCommandType::SetConstantBuffer;
	...
	GL4ConstantBufferCommand& GL4Renderer2DCommandManager::getConstantBuffer(const uint32 index)
	{
	...
	void GL4Renderer2DCommandManager::pushPSTextureUnbind(const uint32 slot)
	{
		...
		const auto id = Texture::IDType::InvalidValue();
		const auto command = ToEnum<GL4Renderer2DCommandType>(FromEnum(GL4Renderer2DCommandType::PSTexture0) + slot);
	...
	void GL4Renderer2DCommandManager::pushPSTexture(const uint32 slot, const Texture& texture)
	{
		assert(slot < SamplerState::MaxSamplerCount);
	...
	const Texture::IDType& GL4Renderer2DCommandManager::getPSTexture(const uint32 slot, const uint32 index) const
	{
		return m_psTextures[slot][index];
	}

	const std::array<Texture::IDType, SamplerState::MaxSamplerCount>& GL4Renderer2DCommandManager::getCurrentPSTextures() const
	{
		return m_currentPSTextures;
	}

	void GL4Renderer2DCommandManager::pushRT(const Optional<RenderTexture>& rt)
	{
		constexpr auto command = GL4Renderer2DCommandType::SetRT;
	...
	const Optional<RenderTexture>& GL4Renderer2DCommandManager::getRT(const uint32 index) const
	{
		return m_RTs[index];
	}

	const Optional<RenderTexture>& GL4Renderer2DCommandManager::getCurrentRT() const
	{
		return m_currentRT;
	}
}
