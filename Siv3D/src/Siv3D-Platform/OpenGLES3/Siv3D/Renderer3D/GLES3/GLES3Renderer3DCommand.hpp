...
namespace s3d
{
	enum class GLES3Renderer3DCommandType : uint32
	{
		Null,
	...
	struct GLES3Renderer3DCommand
	{
		GLES3Renderer3DCommandType type = GLES3Renderer3DCommandType::Null;

		uint32 index = 0;
		
		GLES3Renderer3DCommand() = default;
		
		constexpr GLES3Renderer3DCommand(GLES3Renderer3DCommandType _type, uint32 _index) noexcept
			: type(_type)
			, index(_index) {}
	};

	struct GLES3Draw3DCommand
	{
		uint32 startIndex = 0;
	...
	private:
		...
		// commands
		Array<GLES3Renderer3DCommand> m_commands;
		CurrentBatchStateChanges<GLES3Renderer3DCommandType> m_changes;
