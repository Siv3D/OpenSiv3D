...
namespace s3d
{
	...
	void CMesh_GLES3::init()
	{
		...
		// null Mesh を管理に登録
		{
			...
			// null Mesh を作成
			auto nullMesh = std::make_unique<GLES3Mesh>(vertices, indices);
	...
	Mesh::IDType CMesh_GLES3::create(const Array<Vertex3D>& vertices, const Array<Vertex3D::IndexType>& indices)
	{
		...
		auto mesh = std::make_unique<GLES3Mesh>(vertices, indices);
