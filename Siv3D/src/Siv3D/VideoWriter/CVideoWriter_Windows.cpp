...
namespace s3d
{
	...
	{
		if (auto mf = dynamic_cast<CCodec_MF*>(Siv3DEngine::GetCodec()))
		{
	...
	bool VideoWriter::CVideoWriter::open(const FilePath& path, const Size& frameSize, const double fps)
	{
		if (isOpened())
		{
			close();
	...
	HRESULT VideoWriter::CVideoWriter::initializeSinkWriter(const FilePath& path)
	{
	...
	bool VideoWriter::CVideoWriter::writeFrame(const Image& image, const LONGLONG start, const LONGLONG duration)
	{
