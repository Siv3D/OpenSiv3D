...
namespace s3d
{
	...
	Array<double> BitmapGlyphCache::getXAdvances(const FontData& font, StringView s, const Array<GlyphCluster>& clusters, const double fontSize)
	{
		if (not prerender(font, clusters, true))
		{
			return{};
	...
	double BitmapGlyphCache::xAdvanceFallback(const FontData& font, const GlyphCluster& cluster, const double fontSize)
	{
		if (not prerender(font, { cluster }, false))
		{
			return 0.0;
