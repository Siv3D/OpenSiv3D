...
namespace s3d
{
	...
	RectF MSDFGlyphCache::regionFallback(const FontData& font, const StringView s, const GlyphCluster& cluster, const Vec2& pos, const double size, const double lineHeightScale)
	{
		return regionFallback(font, s, cluster, pos, size, false, lineHeightScale);
	}

	RectF MSDFGlyphCache::regionBaseFallback(const FontData& font, const StringView s, const GlyphCluster& cluster, const Vec2& pos, const double size, const double lineHeightScale)
	{
	...
	RectF MSDFGlyphCache::regionFallback(const FontData& font, const StringView s, const GlyphCluster& cluster, const Vec2& pos, const double size, const bool usebasePos, const double lineHeightScale)
	{
		if (not prerender(font, s, { cluster }))
		{
			return RectF{ 0 };
