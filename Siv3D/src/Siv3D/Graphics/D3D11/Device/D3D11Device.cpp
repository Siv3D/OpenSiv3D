...
namespace s3d
{
	...
	bool D3D11Device::init()
	{
		...
		{
			LOG_FAIL(U"❌ ID3D11Device::QueryInterface() failed");
		...
		{
			LOG_FAIL(U"❌ IDXGIDevice1::GetAdapter() failed");
		...
		{
			LOG_INFO(U"🆗 Graphics adapter: {0}"_fmt(Unicode::FromWString(adapterDesc.Description)));
		}
	...
	bool D3D11Device::initDevice()
	{
		...
		LOG_INFO(U"Available graphics adapters:");

		for (const auto& adapter : adapters)
		{
			LOG_INFO(Unicode::FromWString(adapter.desc.Description));
		}
		...
		if (adapterIndexFromConfig)
		{
				...
				&m_context)))
			{
					...
					|| adapter.vendor == detail::AdapterVendor::Intel)
				{
					...
					LOG_INFO(U"✅ D3D11 device created. Driver type: Hardware (feature level {0})"_fmt(detail::ToString(m_featureLevel)));
				}
				else
				{
					m_driverType = D3D_DRIVER_TYPE_UNKNOWN;

					LOG_INFO(U"✅ D3D11 device created. Driver type: Unknown (feature level {0})"_fmt(detail::ToString(m_featureLevel)));
				}
		...
		if (driverType == D3D_DRIVER_TYPE_WARP)
		{
				...
				&m_context)))
			{
				...
				LOG_INFO(U"✅ D3D11 device created. Driver type: WARP (feature level {0})"_fmt(detail::ToString(m_featureLevel)));
		...
		if (driverType == D3D_DRIVER_TYPE_REFERENCE)
		{
				...
				&m_context)))
			{
				...
				LOG_WARNING(U"✅⚠️ D3D11 device created. Driver type: Reference (feature level {0})"_fmt(detail::ToString(m_featureLevel)));

				return true;
			}
			else
			{
				LOG_FAIL(U"❌ Failed to create D3D11 device (Driver type: Reference)");
		...
		if (driverType == D3D_DRIVER_TYPE_NULL)
		{
				...
				&m_context)))
			{
				...
				LOG_WARNING(U"✅⚠️ D3D11 device created. Driver type: NULL (feature level {0})"_fmt(detail::ToString(m_featureLevel)));

				return true;
			}
			else
			{
				LOG_FAIL(U"❌ Failed to create D3D11 device (Driver type: NULL)");
