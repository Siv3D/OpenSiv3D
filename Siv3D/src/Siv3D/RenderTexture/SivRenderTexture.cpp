...
namespace s3d
{
	...
	{
		Siv3DEngine::GetTexture()->clearRT(m_handle->id(), color);
	}

	void RenderTexture::beginResize()
	{
		Siv3DEngine::GetTexture()->beginResize(m_handle->id());
	}

	bool RenderTexture::endResize(const Size& size, const uint32 multisampleCount)
	{
		return Siv3DEngine::GetTexture()->endResizeRT(m_handle->id(), size, multisampleCount);
	}
}
