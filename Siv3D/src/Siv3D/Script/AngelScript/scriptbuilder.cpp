...
int CScriptBuilder::AddSectionFromFile(const s3d::FilePath& path)
{
	...
	s3d::FilePath fullpath = s3d::FileSystem::FullPath(path);

	if( IncludeIfNotAlreadyIncluded(fullpath.narrow().c_str()) )
	{
...
int CScriptBuilder::LoadScriptSection(const s3d::FilePath& path)
{
	...
	const auto encoding = s3d::Unicode::GetTextEncoding(reader);

	if (encoding == s3d::TextEncoding::UTF8_NO_BOM || encoding == s3d::TextEncoding::UTF8)
	{
		s3d::int32 bomSize = s3d::Unicode::GetBOMSize(encoding);

		code.resize(static_cast<size_t>(reader.size() - bomSize));

		reader.read(&code[0], bomSize, reader.size() - bomSize);
	}
	else
	{
		code = s3d::Unicode::ToUTF8(s3d::TextReader(std::move(reader)).readAll());
	}
	...
	if( includes.size() > 0 )
	{
		...
		else
		{
			...
			for( int n = 0; n < (int)includes.size(); n++ )
			{
				...
				int r = AddSectionFromFile(s3d::Unicode::Widen(includes[n]));				if( r < 0 )
					return r;
			}
		}
	}
