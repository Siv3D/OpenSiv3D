...
int CScriptBuilder::LoadScriptSection(const s3d::FilePath& path, std::vector<std::string>& includedFiles)
{
	...
	if (const auto encoding = s3d::Unicode::GetTextEncoding(reader); 
		(encoding == s3d::TextEncoding::UTF8_NO_BOM) || (encoding == s3d::TextEncoding::UTF8_WITH_BOM))
	{
	...
	{
		code = s3d::TextReader(std::move(reader)).readAll().toUTF8();
	}
