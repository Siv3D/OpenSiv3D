...
namespace s3d
{
	...
	void RegisterCircle(asIScriptEngine* engine)
	{
		...
		r = engine->RegisterObjectMethod(TypeName, "Circle& setCenter(double, double)", asMETHODPR(ShapeType, setPos, (double, double), ShapeType&), asCALL_THISCALL); assert(r >= 0);
		r = engine->RegisterObjectMethod(TypeName, "Circle& setCenter(const Vec2& in)", asMETHODPR(ShapeType, setPos, (const Vec2&), ShapeType&), asCALL_THISCALL); assert(r >= 0);
		...
		r = engine->RegisterObjectMethod(TypeName, "const Circle& drawFrame(double thickness = 0.0, const ColorF& in color = Palette::White) const", asMETHODPR(ShapeType, drawFrame, (double, const ColorF&) const, const Circle&), asCALL_THISCALL); assert(r >= 0);
		...
		r = engine->RegisterObjectMethod(TypeName, "const Circle& drawPie(double startAngle, double angle, const ColorF& in color = Palette::White) const", asMETHOD(ShapeType, drawPie), asCALL_THISCALL); assert(r >= 0);
		r = engine->RegisterObjectMethod(TypeName, "const Circle& drawArc(double startAngle, double angle, double innerThickness = 1.0, double outerThickness = 0.0, const ColorF& in color = Palette::White) const", asMETHOD(ShapeType, drawArc), asCALL_THISCALL); assert(r >= 0);
