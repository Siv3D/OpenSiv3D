...
namespace s3d
{
	...
	void RegisterVec2(asIScriptEngine* engine)
	{
		...
		r = engine->RegisterObjectMethod(TypeName, "Vec2 clampLength(double)", asMETHOD(Vec2, clampLength), asCALL_THISCALL); assert(r >= 0);
		...
		r = engine->RegisterObjectMethod(TypeName, "Vec2 opMul_r(double) const", asFUNCTIONPR(operator*, (double, const Vec2&), Vec2), asCALL_CDECL_OBJLAST); assert(r >= 0);
