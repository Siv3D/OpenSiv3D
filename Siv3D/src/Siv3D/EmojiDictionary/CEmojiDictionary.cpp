...
# include "CEmojiDictionary.hpp"

namespace s3d
{
	EmojiDictionary::CEmojiDictionary::CEmojiDictionary()
	{
	...
	bool EmojiDictionary::CEmojiDictionary::load(const FilePath& path)
	{
		clear();
...

		{
			...
			while (size_to_read)
			{
				...
				codePoints.push_back(std::vector<std::uint32_t>(std::begin(buffer), std::begin(buffer) + length));
			}
		}
	...
	void EmojiDictionary::CEmojiDictionary::clear()
	{
		if (m_trieTree)
		{
			delete m_trieTree;
	...
	size_t EmojiDictionary::CEmojiDictionary::check(String::const_iterator it, const String::const_iterator& itEnd)
	{
