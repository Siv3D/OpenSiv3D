...
void simulate8bit(Bitmap<float> &bitmap) {
    ...
    for (int y = 0; y < h; ++y)
        for (int x = 0; x < w; ++x) {
            unsigned char v = static_cast<unsigned char>(clamp(int(bitmap(x, y)*0x100), 0xff));
            bitmap(x, y) = v/255.f;
        }
}
...
void simulate8bit(Bitmap<FloatRGB> &bitmap) {
    ...
    for (int y = 0; y < h; ++y)
        for (int x = 0; x < w; ++x) {
            unsigned char r = static_cast<unsigned char>(clamp(int(bitmap(x, y).r*0x100), 0xff));
            unsigned char g = static_cast<unsigned char>(clamp(int(bitmap(x, y).g*0x100), 0xff));
            unsigned char b = static_cast<unsigned char>(clamp(int(bitmap(x, y).b*0x100), 0xff));
