...
namespace s3d
{
	...
	public:
		...
		void setNullData(const Data& data)
		{
			...
			LOG_DEBUG(S3DSTR("💠 Created {0}[0(null)]")_fmt(m_assetTypeName));
		}
		...
		IDType add(const Data& data)
		{
			...
			if (!m_idFilled)
			{
				...
				LOG_DEBUG(S3DSTR("💠 Created {0}[{1}]")_fmt(m_assetTypeName, m_idCount));
			...
			{
				LOG_FAIL(S3DSTR("❌ No more {0}s can be created")_fmt(m_assetTypeName));
		...
		void erase(const IDType id)
		{
			...
			LOG_DEBUG(S3DSTR("♻️ Released {0}[{1}]")_fmt(m_assetTypeName, id));
		...
		void destroy()
		{
			for (const auto& data : m_data)
			{
				...
				{
					LOG_DEBUG(S3DSTR("♻️ Released {0}[0(null)]")_fmt(m_assetTypeName));
				}
			}
