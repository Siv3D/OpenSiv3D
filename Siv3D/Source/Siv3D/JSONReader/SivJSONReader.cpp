...
namespace s3d
{
	namespace detail
	{
		...
		{
			const rapidjson::GenericValue<rapidjson::UTF16<>>* pValue = nullptr;

			JSONArrayIteratorDetail() = default;

			explicit constexpr JSONArrayIteratorDetail(const rapidjson::GenericValue<rapidjson::UTF16<>>* p) noexcept
				: pValue(p) {}
		};

		struct JSONMemberIteratorDetail
		{
			rapidjson::GenericValue<rapidjson::UTF16<>>::ConstMemberIterator it;

			JSONMemberIteratorDetail() = default;

			JSONMemberIteratorDetail(const rapidjson::GenericValue<rapidjson::UTF16<>>::ConstMemberIterator& _it)
				: it(_it) {}
		};

		struct JSONValueDetail
		{
			Optional<const rapidjson::GenericValue<rapidjson::UTF16<>>&> value;

			JSONValueDetail() = default;

			JSONValueDetail(const Optional<const rapidjson::GenericValue<rapidjson::UTF16<>>&>& _value)
				: value(_value) {}
		};
	...
	{
		return JSONValue(Optional<const rapidjson::GenericValue<rapidjson::UTF16<>>&>(*(m_detail->pValue)));
	}
	...
	JSONObjectMember JSONObjectIterator::operator *() const
	{
		return{ String(m_detail->it->name.GetString(), m_detail->it->name.GetStringLength()),
			JSONValue(Optional<const rapidjson::GenericValue<rapidjson::UTF16<>>&>(m_detail->it->value)) };
	}
