...
namespace s3d
{
	...
	FontData::FontData(FT_Library library, const String& filePath, const int32 fontSize)
	{
		...
		m_fontSize = fontSize;
		m_lineSpacing = (m_faceText->size->metrics.height / 64);
	...
	bool FontData::render(const std::u32string& codePoints)
	{
		...
		for (const auto& codePoint : codePoints)
		{
			...
			if (glyphIndexText == 0 && glyphIndexEmoji == 0)
			{
				...
				if (!m_tofuIndex)
				{
					if (const FT_Error error = ::FT_Load_Glyph(m_faceText, 0, FT_LOAD_DEFAULT))
					{
						continue;
					}

					const FT_GlyphSlot slot = m_faceText->glyph;

					if (m_faceText->glyph->format != FT_GLYPH_FORMAT_BITMAP)
					{
			...
			else
			{
				...
				Log << codePoint << L" table " << m_glyphs.size() - 1;
				Log << info.bitmapRect;
			}
		}
