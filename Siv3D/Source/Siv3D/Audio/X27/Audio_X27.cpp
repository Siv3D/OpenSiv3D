...
namespace s3d
{
	Audio_X27::Audio_X27(Wave&& wave, AudioDevice_X27* const device, const Optional<std::pair<int64, int64>>& loop, const double maxSpeed)
		: m_device(device)
	...
	bool Audio_X27::isInitialized() const noexcept
	{
		return m_initialized;
	}

	uint32 Audio_X27::samplingRate() const
	{
		return m_wave.samplingRate();
	}

	size_t Audio_X27::samples() const
	{
		return m_wave.size();
	}

	void Audio_X27::setLoop(const bool loop, const int64 loopBeginSample, const int64 loopEndSample)
	{
		m_stream.setLoop(loop, loopBeginSample, loopEndSample);
	}

	bool Audio_X27::changeState(const AudioControlState state, const double durationSec)
	{
	...
	bool Audio_X27::updateFadeManager()
	{
	...
	void Audio_X27::setFadeVolume(const double volume)
	{
	...
	void Audio_X27::setPosSample(int64 posSample)
	{
	...
	uint64 Audio_X27::streamPosSample() const
	{
	...
	std::pair<double, double> Audio_X27::getVolume() const
	{
		return m_stream.getVolume();
	}

	void Audio_X27::setSpeed(const double speed)
	{
		m_stream.setSpeed(speed);
	}

	double Audio_X27::getSpeed() const
	{
		return m_stream.getSpeed();
	}

	std::pair<double, double> Audio_X27::getMinMaxSpeed() const
	{
