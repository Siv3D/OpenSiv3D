...
namespace s3d
{
	...
	static void ConstructU(uint32 rgb, ValueType* self)
	{
	...
	void RegisterColor(asIScriptEngine *engine)
	{
		...
		r = engine->RegisterObjectBehaviour(TypeName, asBEHAVE_CONSTRUCT, "void f(uint32 rgb)", asFUNCTION(ConstructU), asCALL_CDECL_OBJLAST); assert(r >= 0);
