...
namespace s3d
{
	...
	{
		return m_handle->getID() == ScriptHandle::NullID;
	}

	Script::IDType Script::id() const
	{
	...
	{
		return m_handle->getID() == script.m_handle->getID();
	}
	...
	{
		return Siv3DEngine::GetScript()->compiled(m_handle->getID());
	}

	AngelScript::asIScriptFunction* Script::_getFunction(const String& decl) const
	{
		return Siv3DEngine::GetScript()->getFunction(m_handle->getID(), decl);
	}

	std::shared_ptr<ScriptModuleData> Script::_getModuleData() const
	{
		return Siv3DEngine::GetScript()->getModuleData(m_handle->getID());
	}
