		...
		case ClientMessage:
			{
				...
				else if (event->xclient.message_type == _glfw.x11.XdndEnter)
				{
					...
					// [Siv3D] Read XdndTypeList
					bool isFilePath = false;
					int get_types = ((event->xclient.data.l[1] & 1) != 0);
					if(get_types)
					{
						Atom actual_type;
						int actual_format;
						...
						//bytes unread.
						XGetWindowProperty(_glfw.x11.display, event->xclient.data.l[0],
						...
						{
							if(atoms[i] == XInternAtom(_glfw.x11.display, "text/uri-list", False))
							{
								isFilePath = true;
								break;
							}
					...
					else
					{
						for(int i = 0; i < 3; i++)
						{
							if(event->xclient.data.l[2 + i] == XInternAtom(_glfw.x11.display, "text/uri-list", False))
							{
								isFilePath = true;
								break;
							}
