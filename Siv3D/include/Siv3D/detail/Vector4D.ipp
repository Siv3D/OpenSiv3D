...
namespace s3d
{
	...
	template <class Type>
	inline Vector4D<Type> Vector4D<Type>::setLength(const value_type _length) const noexcept
	{
	...
	template <class Type>
	inline Vector4D<Type>& Vector4D<Type>::setLengthSelf(const value_type _length) noexcept
	{
		if (const value_type len = length())
		{
			*this *= (_length / len);
