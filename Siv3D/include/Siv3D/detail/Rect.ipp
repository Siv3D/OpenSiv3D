...
namespace s3d
{
	...
	inline constexpr RectF Rect::scaledAt(const double _x, const double _y, const double s) const noexcept
	{
		return{ (_x + (x - _x) * s), (_y + (y - _y) * s), (w * s), (h * s) };
	}

	inline constexpr RectF Rect::scaledAt(const double _x, const double _y, const double sx, const double sy) const noexcept
	{
		return{ (_x + (x - _x) * sx), (_y + (y - _y) * sy), (w * sx), (h * sy) };
	}

	inline constexpr RectF Rect::scaledAt(const double _x, const double _y, const Vec2 s) const noexcept
	{
